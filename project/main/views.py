...
from .forms import SeriesCreateForm, ArticleCreateForm,SeriesUpdateForm, ArticleUpdateForm, NewsletterForm, ContactForm
...
def contact(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            reden = form.cleaned_data['reden']

 
            html = render_to_string('main/contactformulier.html', {
                'firstname':firstname,
                'lastname':lastname,
                'email':email,
            ...
            mail = EmailMessage(reden, html,to=["herbertingels@hotmail.com"],bcc=[email,])
            mail.content_subtype = 'html'
            #return redirect('success') 
        #else:
            
            if mail.send():
               messages.success(request, "Email is succesvol verstuurd!")
               return redirect('index')
            else:
                messages.error(request, "Er was een probleem met het versturen van de email!")
...
def homepage(request):
    ...
    return render(request, template_name='main/opleiding.html',context={"objects":matching_series, "type":"series"})

@login_required     
def series(request,series:str):
    matching_series = Article.objects.filter(series__slug=series)
    
    return render(request, template_name='main/lessen.html',context={"objects":matching_series,"type":"article"})
...
def newsletter(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            mail = EmailMessage(subject, email_message, f"Iloveswimming <{request.user.email}>", bcc=receivers)
            mail.content_subtype = 'html'
